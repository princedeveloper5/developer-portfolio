    ...
    <div class="split-right">
      <div class="split-form-container">
        ...
        <form @submit.prevent="handleReset">
          ...
          <p v-if="message" class="split-success">{{ message }}</p>
          <p v-if="error" class="split-error">{{ error }}</p>
        </form>

        <RouterLink to="/login" class="split-back-link">← Back to login</RouterLink>
      </div>
    </div>
  </section>
</template>
...
async function handleReset() {
  ...
  try {
    await api.requestPasswordReset(form.username);
...
<style scoped>
/* Split auth page styles - shared with AuthLoginView */
.split-auth-page {
  display: grid;
